...
function Code({ children }) {
  return (
    <code className="mt-1 block select-all whitespace-pre-wrap break-all rounded bg-muted/30 px-3 py-2 font-mono text-xs text-primary">
      {children}
    </code>
  )
}
  ...
  return (
    <div className="max-w-2xl space-y-6 p-6">
      <div className="flex items-start gap-3 rounded-lg border border-destructive bg-destructive/10 p-4">
        ...
        <div>
          <div className="font-semibold text-destructive">Cannot reach NATS server</div>
          <div className="mt-1 font-mono text-sm text-destructive/80">{cleanError || error}</div>
        </div>
      </div>
