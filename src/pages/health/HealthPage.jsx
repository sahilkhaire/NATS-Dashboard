    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <div className="text-sm text-muted-foreground">Last check: {lastFetch ? new Date(lastFetch).toLocaleTimeString() : '-'}</div>
        </div>
      </div>
      {error && <AlertBanner variant="error" title="Health check failed">{error}</AlertBanner>}
      {data && <pre className="overflow-auto rounded bg-card p-4 text-xs text-foreground">{JSON.stringify(data, null, 2)}</pre>}
    </div>
  )
}
